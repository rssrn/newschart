 ...
 *
 * Examples:
 *   node screenshot-tests.js --bounds
 *   node screenshot-tests.js -t 2-us-cluster,3-asian-cluster
...
function parseArgs() {
  ...
  return { testCaseNames, showBounds };
}

async function takeScreenshots() {
  const { testCaseNames, showBounds } = parseArgs();
  ...
  for (const testName of testCaseNames) {
    ...
    // Navigate (with optional bounding box)
    const boundsParam = showBounds ? '?showBoundingBox=true' : '';
    const url = `${FRONTEND_URL}${boundsParam}`;
    ...
    console.log(`✓ Captured: ${testName}.png`);
