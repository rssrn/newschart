...
export interface SourceMeta {
  ...
  shortLabel: string;
  color: string;
  /** SVG path data. If absent, falls back to letter. */
  svgPath?: string;
  /** viewBox for the SVG; defaults to "0 0 24 24" (simple-icons standard). */
  svgViewBox?: string;
}

// OpenAI "Blossom" icon mark, extracted from openai.com/brand (left panel, fill path only).
// Coordinate bounds: x 146–414, y 227–492.
...
export const SOURCE_META: Record<CalloutSource, SourceMeta> = {
  // Brand colors from simple-icons (official submissions). B&W brands use mid-gray so hollow
  // badges remain legible on the dark map background.
